...
> 请遵守所在地区法律，并尊重目标站点的访问频率限制。

## 能做什么

| 功能 | 说明 |
| --- | --- |
| 按车牌号下载 | 输入 `438696` / `JM438696` / 完整 18comic 链接都能识别；支持一次填多个 |
...
到 [Releases](https://github.com/NanNank0/JMComic_downloader/releases/latest) 下载对应平台的包，**不需要手动安装 Python**。

| 平台 | 产物 |
| --- | --- |
| **Windows** | `jmcomic-windows-x64.zip` |
| **Linux** | `jmcomic-linux-x86_64.tar.gz` |
| **macOS** | `jmcomic-macos.zip` |
| **Android** | `jmcomicdownloader-*.apk` |
...
python webui/server.py  # 此脚本会自动打开浏览器
```
...
```powershell
python scripts/jmctl.py info 350234                 # 查元数据
python scripts/jmctl.py search "董卓"               # 搜索
python scripts/jmctl.py rank --period week          # 周榜
python scripts/jmctl.py download 350234 --export pdf  # 下载 + 导出 PDF
```
...
```powershell
"C:\打印出来的路径\python.exe" -m pip install jmcomic
```
...
输出：成功是 `{"status":"ok", ...}`，失败是 `{"status":"error","error":"...","hint":"..."}`，并且**退出码为 1**，便于脚本判断。

---

## 配置

配置优先级：**命令行参数 / 界面设置 > `option.yml` > 默认值**。
...
本地服务绑定 `localhost`，并且每次启动生成一个随机 token：
...
**下载引擎（`jmcore.py`）与界面分离**。引擎不依赖任何 GUI 库、也不需要终端，所以 CLI、网页界面、Android APK 能共用它。网页界面之所以可行，也正因为引擎本身就能在没有界面的环境里跑。

~~主要是因为这样可以只维护一套代码很方便耶~~

---

## 常见问题

不知道有什么问题，有人提了 issue 再写吧
...
- 下载能力来自 [JMComic-Crawler-Python](https://github.com/hect0x7/JMComic-Crawler-Python)，本项目与该项目及 JMComic 网站**没有任何隶属关系**。
